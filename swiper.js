(function () {
	function Swiper(option) {
		...
		this.showIndex = 0;
		this.showImgNum =
			this.imageWidth && Math.floor(this.showWidth / this.imageWidth);
		this.showImgGorp =
			this.imageWidth == undefined
				? this.imageNum
				: this.imageNum / this.showImgNum;
		this.lock = true;
		this.time;
		...
		Swiper.prototype.addDom = function () {
			...
			if (this.animateType == "animate") {
				for (
					var n = 0;
					n <
					(this.imageWidth == undefined ? 1 : this.imageNum / this.showImgNum);
					n++
				) {
					$(
						'<li><a href="#"><img src="' + this.imageList[n] + '"></a></li>'
					).appendTo($ul);
				}
			}
		...
		Swiper.prototype.bindEvent = function () {
			...
			$(".rightBtn", this.wrap).click(function () {
				if (self.animateType == "animate" && self.lock) {
					self.showIndex++;
					self.lock = false;
					$(".innerWrap", self.wrap).animate(
						{ left: -self.showIndex * self.showWidth },
						300,
						"swing",
						function () {
			...
			$(".leftBtn", this.wrap).click(function () {
				if (self.animateType == "animate" && self.lock) {
					...
					self.lock = false;
					if (self.showIndex < 0) {
						$(".innerWrap", self.wrap).css({
							left: -self.showImgGorp * self.showWidth,
						});
